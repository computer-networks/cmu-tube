...
echo "=== (unrestricted-1mbit, 100ms latency, 1min) profile ==="
...
echo "[PHASE 1] Keeping unrestricted bandwidth for 10 seconds..."
sleep 10

# --- (7) Remove bandwidth limits to make link unrestricted ---
echo "[PHASE 2] Add 1mbit bandwidth limits ..."
toxiproxy-cli toxic add --upstream   --type bandwidth --attribute rate=125 video
toxiproxy-cli toxic add --downstream --type bandwidth --attribute rate=125 video

echo "[INFO] After removing bandwidth toxics:"
toxiproxy-cli inspect video

# --- (8) Keep unrestricted for another 50 s ---
echo "[INFO] Keeping 1mbit bandwidth for 50 seconds..."
sleep 55
...
pkill -9 -f toxiproxy || true
echo "=== (unrestricted-1mbit, 100ms latency, 1min) finished ==="
